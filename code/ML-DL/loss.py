...
class MLDL_Loss(object):
    ...
    def DistanceLoss(self, data, latent, dis_data, dis_latent, kNN_data, kNN_latent):
        ...
        if 'MNIST' in self.args['DATASET']:
            ...
            Error2 = (0 - D2_1) / 1
        #     print("Error2 : ", Error2)
        # print("Printing value 1 \n ", torch.sum(kNN_data == False))
        # print("KNN Data : ", kNN_data)
        # print("Error2 : ", Error2[Error2 > -1 * self.args['RegularB']])
        loss_push_away = torch.norm(Error2[Error2 > -1 * self.args['RegularB']]) / (torch.sum(kNN_data == False) + 1e-10)
